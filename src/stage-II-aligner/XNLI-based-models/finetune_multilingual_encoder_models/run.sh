...
FULL_MODEL_NAME=$($PYTHON -c "print('$FULL_MODEL_NAME'.replace('/', '-'))+'-'+'$EPOCHS'+'-'+'$LR'")
...
echo "FULL_MODEL_NAME: $FULL_MODEL_NAME"
...
# execute training
$PYTHON $MODEL_DIR/main.py --dataset_path $DATASET_DIR --epochs $EPOCHS --gpus $GPUS --batch_size $BATCH_SIZE --max_seq_len $MAX_SEQ_LENGTH --checkpoint_path $CHECKPOINT_PATH --learning_rate $LR --model_name $MODEL_NAME --online_mode $ONLINE_SYNC --fp16 $MIXED_PRECISION --use_pretrained $PRETRAINED
