...
class ModelWrapper(pl.LightningModule):
    def __init__(self, args):
        ...
        self.tokenizer = MT5Tokenizer.from_pretrained(args.model_name, cache_dir='/tmp/huggingface')
        if args.use_pretrained:
            # using pretrained transformers
            self.model = MT5ForConditionalGeneration.from_pretrained(args.model_name, dropout_rate=args.dropout_rate, cache_dir='/tmp/huggingface')
        else:
...
if __name__ == "__main__":
    ...
    args.logger_exp_name = "ds-%s-%s-%s-%s" % ('-'.join(args.lang), args.model_name, args.epochs, args.learning_rate)
    ...
    logger_args = {
        'project': 'stage-II-distant-supervision',    # first create a project on weight & bias with local account
